...
const VehiclesCausingExcessiveNoisePage = () => {
  const { content, loading, error } = useK53Data('Rules Of The Road', 'Vehicles causing excessive noise');
  ...
  useEffect(() => {
    if (content) {
      ...
      setLines(splitLines);
      setChecked(new Array(splitLines.length).fill(false));
    }
  }, [content]);
  ...
  return (
    ...
    >
      <VerticalLineContainer className="flex flex-col items-center mt-32 px-4 max-w-3xl mx-auto w-full">
          ...
          {lines.map((line, i) => (
            ...
            >
              ...
              <button
                ...
                aria-label={checked[i] ? 'Mark as unread' : 'Mark as read'}
                type="button"
              >
                {checked[i] ? <FaCheckCircle size={24} /> : <FaRegCircle size={24} />}
              </button>
            </div>
          ))}
        </div>
        ...
        <StyledButton
          to="/vehicles-causing-excessive-noise/quiz"
          asLink
          variant="large"
...
export default VehiclesCausingExcessiveNoisePage;
